...
def svm_loss_naive(W, X, y, reg):
  ...
  for i in range(num_train):
    ...
    for j in range(num_classes):
      ...
      if margin > 0:#0보다 작다면 MAX값은 0이니까 아무것도 안하면 됨 
        loss += margin#전체 평균구하고 정규화할 때 쓰려고
        dW[:, y[i]] = dW[:, y[i]] - X[i] 
        dW[:,j] = dW[:,j] + X[i]    
...
def svm_loss_vectorized(W, X, y, reg):
  ...
  margin = np.maximum(0, scores - correct_scores + 1)#정답을 맞췄다면 무조건 1
  ...
  #############################################################################
  # Compute gradient
  margin[margin > 0] = 1 #정답 데이터보다 더 크게 나온 label값을 1로 만들기 --> dW[:,j] = dW[:,j] + X[i](이 상태 그대로 두면 됨)
  valid_margin_count = margin.sum(axis=1) #정답 데이터보다 더 크게 나온 label값 갯수만큼 더해짐 
    
  margin[np.arange(num_train),y ] -= valid_margin_count  
  #dW[:, y[i]] = dW[:, y[i]] - X[i] :Subtract in correct class(correct_class는 num_train수 만큼 빼야하니까)
